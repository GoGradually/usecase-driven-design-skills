...
4단계 도메인 모델과 2단계 시나리오를 읽고, 기존 상태 모델을 참조하여 확장한다.
...
- 이 기능으로 추가되는 상태: [REFUND_REQUESTED, REFUNDED]

## 기존 상태 모델 확장

| 엔티티 | 추가 상태 | 추가 전이 | 사유 |
|--------|----------|----------|------|
| Order | REFUND_REQUESTED | PAID → REFUND_REQUESTED | 환불 기능 추가 |
| Order | REFUNDED | REFUND_REQUESTED → REFUNDED | 환불 완료 |
...
**주의사항:**
- 기존 상태 모델이 있으면 기존 상태와 전이를 먼저 명시하고, 신규 상태/전이만 추가한다.
- 신규 전이가 기존 상태를 출발점으로 삼을 수 있다.
