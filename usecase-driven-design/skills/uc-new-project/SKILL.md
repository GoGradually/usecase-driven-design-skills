---
name: uc-new-project
description: 새로운 프로젝트를 시작할 때 UseCase 설계 초안을 만든다. 프로젝트 디렉토리를 생성하고, 8단계 워크플로우(유스케이스 작성 → 시나리오 → 변수 식별 → 도메인 모델 → 상태 모델 → 시스템 경계(UC 다이어그램 포함) → 예외 정리 → 사전/사후조건)를 단계별 확인을 받으며 진행한다. 각 단계 산출물은 개별 마크다운 파일에 기록하고, 완료 후 하나의 초안 파일로 통합한다. '새 프로젝트', '프로젝트 시작', '처음부터 유스케이스', 'UseCase 새로 만들기' 등의 요청에 사용한다. 기존 프로젝트에 기능을 추가하는 경우에는 uc-add-feature 스킬을 사용한다.
---
...
**8단계 워크플로우**를 순서대로 진행하며, 각 단계 산출물을 **개별 마크다운 파일**에 기록한다.
...
모든 단계의 산출물에는 **Mermaid 다이어그램을 필수로 포함**한다. (단, 1단계는 다이어그램 없음)
...
|---------|---------|--------|----------|------|
| Order | OrderItem | 1:N | 합성(composition) | 주문 삭제 시 주문항목도 삭제 |
| Order | Customer | N:1 | 연관(association) | 고객 삭제해도 주문 이력 유지 |

**관계 유형 정의:**
- **연관(association)**: 독립적 생명주기. A 삭제해도 B 유지
- **집약(aggregation)**: 전체-부분 관계이나 부분이 독립 존재 가능
- **합성(composition)**: 전체-부분 관계이며 부분이 전체에 종속 (전체 삭제 시 부분도 삭제)

## 일반화/특수화 (IS-A 관계)

엔티티 간 상속 관계가 있는 경우 식별한다. 해당 없으면 "해당 없음"으로 기재한다.

| 상위 엔티티 | 하위 엔티티 | 구분 기준 | 비고 |
|------------|-----------|----------|------|
| PaymentMethod | CreditCard, BankTransfer | 결제 수단 유형 | 완전/배타적 |

**분류 체계 표기:**
- **완전(complete)**: 모든 인스턴스가 하위 타입 중 하나에 속함
- **불완전(incomplete)**: 하위 타입에 속하지 않는 인스턴스 가능
- **배타적(disjoint)**: 하위 타입 간 중복 없음
- **중첩(overlapping)**: 하위 타입 간 중복 가능
...
각 UC의 시나리오를 분석하여 세 가지 분석 객체로 분해한다.

### UC-01: [유스케이스명]

| 구분 | 분석 객체 | 설명 | 시나리오 근거 |
|------|----------|------|-------------|
| Boundary | 주문 화면 | 사용자가 주문 정보를 입력하는 인터페이스 | 기본흐름 1단계 |
| Control | 주문 처리기 | 주문 생성 로직을 조정 | 기본흐름 2~3단계 |
| Entity | Order | 주문 도메인 객체 | 엔티티 목록 참조 |
...
- 관계 유형(연관/집약/합성)을 생명주기 의존성 기준으로 분류한다. Mermaid classDiagram에서 연관 `-->`, 집약 `o--`, 합성 `*--`, 상속 `<|--`로 표기한다.
- 엔티티 간 IS-A 관계(타입 계층)가 있는지 검토한다. 공통 속성이나 행위를 공유하는 엔티티가 있으면 상위/하위 관계를 식별한다.
- BCE 분석에서 Boundary는 시나리오에서 액터가 직접 상호작용하는 지점(UI 화면, API 엔드포인트), Control은 "시스템이 ~한다"로 기술된 조정/처리 로직, Entity는 도메인 모델의 엔티티와 1:1 대응한다. 하나의 UC에 Boundary, Control은 각각 최소 1개 이상이어야 한다.
...
**핵심 흐름:**
1. 시나리오의 동사에서 **비즈니스 이벤트**를 먼저 식별한다.
2. 각 이벤트가 어떤 엔티티에 **어떤 변화(전이)** 를 일으키는지 분석한다.
3. 전이를 모으면 **상태가 자연스럽게 도출**된다.
4. 도출된 상태들로 전체 생애주기를 확정하고, 불허 전이를 정의한다.
...
**주의사항:**
- **이벤트가 출발점이다.** 상태를 먼저 나열하지 않는다. 시나리오의 동사(행위)에서 이벤트를 식별하고, 이벤트가 일으키는 전이를 통해 상태를 도출한다.
- 이 모델은 **개념 수준**(비즈니스 이벤트, 비즈니스 상태)이다. 구현 수준(DB 컬럼, 이벤트 큐, 메시지 브로커)은 다루지 않는다.
- 하나의 이벤트가 여러 출발 상태에서 서로 다른 도착 상태를 만들 수 있다.
- 하나의 엔티티가 여러 UC에 걸쳐 전이되는 경우, 모든 관련 UC를 근거에 포함한다.
- 도출된 상태에서 "이 전이는 비즈니스적으로 있어서는 안 된다"를 불허 전이로 명시한다.

### 암묵적 상태 도출 체크리스트

시나리오에 명시적으로 드러나지 않지만, 백엔드 검증/처리에 필요한 상태를 놓치지 않기 위해
아래 질문을 각 엔티티에 적용한다:

1. **검증 대기 상태**: 이 엔티티가 생성/변경된 후, 다음 UC에서 사용되기 전에
   유효성 검증이나 승인 절차가 필요한가? → 중간 상태 도입
2. **처리중 상태**: 외부 시스템 연동이나 비동기 처리가 있을 때,
   요청~완료 사이에 "처리중" 상태가 필요한가?
3. **조건부 전이**: 특정 조건(결제 완료, 재고 확인 등)이 충족되어야만
   다음 상태로 넘어가는가? → 대기/보류 상태 도입
4. **되돌림 불가 경계**: 이 전이 이후에는 복귀할 수 없는 지점이 있는가?
   → 전이 전 확인 상태가 필요할 수 있음

도출된 암묵적 상태는 "2. 이벤트-전이 매핑" 테이블에 추가하고,
근거 컬럼에 "검증 필요" / "비동기 처리" 등 도출 사유를 기재한다.

**Mermaid: 상태 다이어그램** — `stateDiagram-v2`로 작성. 상태명은 한글, 전이 레이블에 트리거 이벤트를 표기한다.
...
- 5단계 상태 모델의 불허 전이도 예외 상황의 근거가 된다.
...
**주의사항:**
- 사전조건에 5단계 상태 모델의 "출발 상태"를 반영한다 (예: "주문 상태가 CREATED여야 한다").
- 사후조건에 5단계 상태 모델의 "도착 상태"를 반영한다 (예: "주문 상태가 CONFIRMED으로 변경된다").
