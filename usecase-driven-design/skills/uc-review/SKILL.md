...
| 상태 모델 존재 여부 | 라이프사이클 엔티티에 대한 상태 모델이 작성되었는가? 상태 목록, 허용 전이, 불허 전이, 비즈니스 이벤트가 포함되었는가? |
...
| 상태-시나리오 정합성 | 전이 트리거의 UC 시나리오 근거가 실제 시나리오 단계와 일치하는가? |
...
| 상태 모델 완전성 | 모든 라이프사이클 엔티티의 모든 도달 가능 상태가 포함되었는가? |
| 전이 트리거 구체성 | 비즈니스 이벤트가 구체적이고, UC 시나리오 단계가 정확히 연결되었는가? |
