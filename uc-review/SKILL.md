...
| 항목 | 검토 내용 |
|------|----------|
| 7단계 존재 여부 | 유스케이스 목록, 시나리오, 변수 식별, 도메인 모델, 경계 분리, 예외, 사전/사후조건 모두 있는가? |
...
| Mermaid 다이어그램 | 1~5단계에 필수 다이어그램이 모두 포함되어 있는가? |
...
| 액터 일관성 | 시나리오의 액터가 5단계 액터 목록에 모두 포함되어 있는가? |
...
| 다이어그램 간 정합성 | 1단계↔5단계, 4단계↔3단계 다이어그램이 서로 대응하는가? |
