...
4단계 도메인 모델(`step4-domain-model.md`)과 2단계 시나리오(`step2-scenarios.md`)를 읽고, 라이프사이클(상태 전이)을 가지는 엔티티를 식별한 뒤 개념 수준의 상태 모델을 작성한다.
...
| 엔티티 | 라이프사이클 여부 | 근거 |
|--------|----------------|------|
| Order  | ✅ | 시나리오에서 생성→확인→결제→배송 등 상태 전이 발생 |
| Product | ❌ | 상태 전이 없음 (참조 데이터) |
...
| 상태 | 설명 | 진입 조건 |
|------|------|----------|
| CREATED | 주문이 생성됨 | UC-01 기본흐름 3단계 |
| CONFIRMED | 주문이 확인됨 | UC-02 기본흐름 2단계 |

### 허용 전이

| 출발 상태 | 도착 상태 | 트리거 (비즈니스 이벤트) | UC 시나리오 근거 |
|----------|----------|----------------------|----------------|
| CREATED | CONFIRMED | 주문 확인 요청 | UC-02 기본흐름 2단계 |
| CREATED | CANCELLED | 주문 취소 요청 | UC-03 기본흐름 1단계 |
...
**주의사항:**
- 이 모델은 **개념 수준**(비즈니스 상태, 비즈니스 이벤트)이다. 구현 수준(DB 컬럼, 이벤트 큐)은 다루지 않는다.
- 시나리오를 역추적하여 각 상태의 진입 조건과 전이 트리거를 UC 시나리오 단계에 연결한다.
